...
const ListDokumenLain = ({item, tipe, token}) => {
...
export const DokumenLain = () => {
    ...
    useEffect(() => {
        setType('dokumen_lain')
        dispatch(getListComposer({token:token, tipe:type}));
    }, [type])
    ...
    const filterHandlerInProgress = () => {
        setTipe('inprogress')
        dispatch(getListInProgress({token:token, tipe:'dokumen_lain'}));
    }
    const filterHandlerCompleted = () => {
        setTipe('')
        dispatch(getListCompleted({token:token, tipe:'dokumen_lain'}));
    }
    const filterHandlerDraft = () => {
        setTipe('')
        dispatch(getListDraft({token:token, tipe:'dokumen_lain'}));
    }
                    ...
                    >
                        <BottomSheetView onLayout={handleContentLayout}>
                            <View>
                                ...
                                <View style={{ marginHorizontal: 20, gap: 30, alignItems: "center", marginTop: 50}}>
