...
export const AddTask = ({ route }) => {
    ...
    const { status, treeView, detailProject, attachment } = useSelector(state => state.task)
                    ...
                    <View style={styles.Card}>
                        <View style={{ flexDirection: 'column', gap: 6 }}>
                            <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
                                ...
                                <Text style={{ flex: 1, fontSize: FONTSIZE.H3 }}>{dataTask.project}</Text>
                            </View>

                            <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
                                <Text style={{ width: 100, fontWeight: FONTWEIGHT.bold, fontSize: FONTSIZE.H3 }}>List</Text>
                                <Text style={{ flex: 1, fontSize: FONTSIZE.H3 }}>{dataTask.list}</Text>
                            </View>
                        </View>
                ...
                <View style={{ justifyContent: 'center', alignItems: 'center', flex: 1 }}>
                    <View style={{ backgroundColor: COLORS.white, width: '90%', height: '70%', borderRadius: 8, padding: 20 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            <Text style={{ fontWeight: FONTWEIGHT.bold }}>Penanggung Jawab</Text>
                        </View>
                        {/* custom divider */}
                        ...
                        <View>
                            <View style={{ marginVertical: 20 }}>
                                <Search onSearch={(e) => {
                                    const data = dataTask.member.filter(item => item.nama.toLowerCase().includes(e.toLowerCase()))
                                    if (e !== '' || data.length > 0) {
