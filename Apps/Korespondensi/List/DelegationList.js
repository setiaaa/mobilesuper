...
function DelegationList() {
  ...
  async function getDelegation() {
    ...
    try {
      ...
      let response;
      response = await getHTTP(nde_api.delegation);
      setList(response.data.results);
      setIsLoading(false);
    } catch (error) {
      setIsSearchFilter(false);
      handlerError(error, "Warning!", "Delegation List not working");
      setIsLoading(false);
    }
  }
  async function getDelegationSearch() {
    setIsLoading(true);
    try {
      let response;
      response = await getHTTP(
        nde_api.delegationsearch.replace("{$word}", searchQuery)
      );
      if (response) {
    ...
    <>
      ...
      <View style={{ flex: 1 }}>
        ...
        {list?.length == 0 && isSearchFilter && (
          <Text style={styles.titleNotFound}>Delegation not found</Text>
        )}
        <FlatList
          data={list}
